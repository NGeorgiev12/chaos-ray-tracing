	...
	const CRTScene& scene, int depth)
{
	...
	else if (reflectedMaterial.getType() == MaterialType::CONSTANT) 
	{
		return CRTVector::multiplyColors(reflectedMaterial.getAlbedo(), albedo);
	}
	else if (reflectedMaterial.getType() == MaterialType::REFRACTIVE) 
	{
		CRTVector refractedColor = CRTRefracter::handleRefraction(reflectedRay, reflectionResult, scene, depth + 1);
		return CRTVector::multiplyColors(refractedColor, albedo);
	}
