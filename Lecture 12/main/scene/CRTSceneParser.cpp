...
CRTMaterial CRTSceneParser::parseMaterial(const Value& jsonMaterial)
{
	...
	const Value& textureName = jsonMaterial.FindMember(CRT_SCENE_MATERIAL_ALBEDO)->value;
	assert(!textureName.IsNull() && textureName.IsString());
	...
	return CRTMaterial(type.GetString(), std::move(textureName.GetString()), smoothShading.GetBool());
}
...
CRTMesh CRTSceneParser::parseMesh(const Value& jsonMesh)
{
	...
	assert(!uvsArr.IsNull() && uvsArr.IsArray());
	...
	return CRTMesh(std::move(vertices), std::move(uvs), 
		std::move(triangles), materialIndex.GetInt());
}
...
std::vector<CRTexture> CRTSceneParser::parseTextures(const Document& jsonDocument)
{
	...
	assert(!texturesArr.IsNull() && texturesArr.IsArray());
