...
private:
	...
	static CRTVector shadeDiffuse(const CRTRay& ray, const CRTIntersectionResult& result,
		const CRTScene& scene, TraceType traceType);
	static CRTVector shadeGlobalIllumination(const CRTRay& ray, const CRTIntersectionResult& result,
		const CRTScene& scene, TraceType traceType);
	static CRTVector shadeDirectIllumination(const CRTRay& ray, const CRTIntersectionResult& result,
		const CRTScene& scene, TraceType traceType);
