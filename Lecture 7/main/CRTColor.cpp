...
{
	// we assume that the CRTVector has normalized coordinates in the range [0, 1]
	redComponent = static_cast<int>(vec.getVertex().x * maxColorComponent);
	greenComponent = static_cast<int>(vec.getVertex().y * maxColorComponent);
	blueComponent = static_cast<int>(vec.getVertex().z * maxColorComponent);
}
