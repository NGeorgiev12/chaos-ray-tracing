...
CRTVector CRTShader::shadeDiffuse(const CRTRay & ray, const CRTIntersectionResult & result, const CRTScene & scene)
{
	...
	const CRTexture& texture = scene.getTextures()[result.textureIndex];
	CRTVector albedo = CRTAlbedo::albedo(texture, result);
...
{
	return CRTVector();//scene.getMaterials()[data.materialIndex].getAlbedo();
}
