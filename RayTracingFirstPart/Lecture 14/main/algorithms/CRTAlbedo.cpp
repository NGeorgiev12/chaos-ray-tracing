...
CRTVector CRTAlbedo::albedo(const CRTScene& scene, const CRTIntersectionResult& result)
{
    const auto& textures = scene.getTextures();
    if (textures.empty())
    {
        return scene.getMaterials()[result.materialIndex].getAlbedo();
    }

    const CRTexture& texture = textures[result.textureIndex];
