	...
	CRTIntersectionResult& result)
{
	...
	const auto& material = scene.getMaterials()[materialIndex];
	bool backFaceCulling = material.getBackFaceCulling();

	const CRTVector& normal = triangle.normalVector();
	const CRTVector& v0 = triangle.getVertices()[0];

	/*bool cullBackFace = (ray.getRayType() == RayType::CAMERA) || backFaceCulling;

	if (cullBackFace)
	{
		if (dotProduct(rayDir, normal) > 0.f)
		{
			return false;
		}
	}*/
