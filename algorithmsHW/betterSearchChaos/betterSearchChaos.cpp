...
static void betterSearch(const AlignedIntArray& hayStack, const AlignedIntArray& needles, AlignedIntArray& indices, StackAllocator& allocator) {
	...
	for (int q = 0; q < needles.getCount(); q++) {
		...
		int left = 0;
		int right = hayStack.getCount();

		while (left < right) {

			int middleIndex = left + (right - left) / 2;

			if (hayStack[middleIndex] < value)
			{
				left = middleIndex + 1;
			}
			else
	...
	//Query* queries = allocator.alloc<Query>(q);

	//if (!queries)
	//	return;

	//for (int i = 0; i < q; i++) {
	//	queries[i].value = needles[i];
	//	queries[i].originalIndex = i;
	//}
	...
	//int leftHayStack = 0;
	//int rightHayStack = hayStack.getCount();
	// 
	//for (int i = 0; i < q; i++) {

	//	int val = queries[i].value;
	//	int originalIndex = queries[i].originalIndex;

	//  int left = leftHayStack;
	//  int right = rightHayStack;
	// 
	//	while (left <= right) {

	//		int middleIndex = left + (right - left) / 2;
	//		
	//		if (hayStack[middleIndex] < val) {
	//			left = middleIndex + 1;
	//			leftHayStack = left;
	//		} else { 
	//			right = middleIndex;
	...
	//	if (left < n && hayStack[left] == val)
	//		indices[originalIndex] = left;
	//	else
